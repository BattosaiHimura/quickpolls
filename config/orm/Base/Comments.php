...
abstract class Comments implements ActiveRecordInterface
{
    ...
    public function setVoteId($v)
    {
        ...
        if ($this->vote_id !== $v) {
            $this->vote_id = $v;
            $this->modifiedColumns[CommentsTableMap::COL_VOTE_ID] = true;
        }

        if ($this->aVotes !== null && $this->aVotes->getId() !== $v) {
            $this->aVotes = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CommentsTableMap::translateFieldName('VoteId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->vote_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CommentsTableMap::translateFieldName('Comment', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = CommentsTableMap::NUM_HYDRATE_COLUMNS.
    ...
    {
        if ($this->aVotes !== null && $this->vote_id !== $this->aVotes->getId()) {
            $this->aVotes = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aVotes !== null) {
                if ($this->aVotes->isModified() || $this->aVotes->isNew()) {
                    $affectedRows += $this->aVotes->save($con);
                }
                $this->setVotes($this->aVotes);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CommentsTableMap::COL_VOTE_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aVotes) {
                ...
                $result[$key] = $this->aVotes->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CommentsTableMap::COL_VOTE_ID)) {
            $criteria->add(CommentsTableMap::COL_VOTE_ID, $this->vote_id);
        }
        if ($this->isColumnModified(CommentsTableMap::COL_COMMENT)) {
    ...
    /**
     * Declares an association between this object and a ChildVotes object.
     *
     * @param  ChildVotes $v
     * @return $this|\Comments The current object (for fluent API support)
     * @throws PropelException
     */
    public function setVotes(ChildVotes $v = null)
    {
        if ($v === null) {
            $this->setVoteId(NULL);
        } else {
        ...
        // If this object has already been added to the ChildVotes object, it will not be re-added.
        if ($v !== null) {
            $v->addComments($this);
        }
    ...
    /**
     * Get the associated ChildVotes object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildVotes The associated ChildVotes object.
     * @throws PropelException
     */
    public function getVotes(ConnectionInterface $con = null)
    {
        if ($this->aVotes === null && ($this->vote_id !== null)) {
            $this->aVotes = ChildVotesQuery::create()->findPk($this->vote_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aVotes->addCommentss($this);
             */
        }
    ...
    {
        if (null !== $this->aVotes) {
            $this->aVotes->removeComments($this);
        }
        $this->id = null;
        $this->vote_id = null;
        $this->comment = null;
